...
public class AController implements Initializable {
    ...
    private void btnFiltreClicked(ActionEvent event) {
        // Récupération du type sélectionné dans la ChoiceBox
        String Ty = choixType.getValue();

        // Récupération de la ville saisie dans le TextField
        AnnonceCRUD AC = new AnnonceCRUD();

        // Récupération de la liste complète des annonces
        List<Annonce> list = AC.filterByType(Ty);

        // Création d'une liste observable pour afficher les annonces filtrées
        ObservableList<Annonce> annoncesFiltrees = FXCollections.observableArrayList();

        // Filtrage des annonces en fonction du type 
        for (Annonce annonce : list) {
            if (annonce.getType().equals(Ty)) {
                annoncesFiltrees.add(annonce);
            }
        }

        // Création des panneaux d'annonces pour chaque annonce filtrée
        vbox.getChildren().clear();
        for (Annonce annonce : annoncesFiltrees) {
            AnchorPane annoncePane = new AnchorPane();
            annoncePane.setPrefSize(600, 200); // taille de chaque annoncePane
